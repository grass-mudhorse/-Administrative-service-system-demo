...
<c:forEach items="${tours}" var="item">
	<tr class="rowTr">
		...
		<td>${item.tourage}</td>
		<td>${item.toursex}</td>
		<td>${item.tourphone}</td>
		<td>${item.tourcon}</td>
		<td>${item.tourtempcon}</td>
		<td>
			<a class="btn btn-link btn-edit" href="javascript:location.href='${ctx}/tourController/toedit.do?id=${item.id}';">修改</a>
			<a class="btn btn-link btn-delete" href="javascript:;" onclick="deletetour('${item.id}')">删除</a>
		</td>
	</tr>
</c:forEach>
