...
public abstract class BaseController {
	...
	public void setPaging(int pagesize, int sumrow, int currentpage){
		...
		if(sumpage<1){//如果总页数小�?1，则赋�?�为1
			sumpage = 1;
		}
		if(currentpage < 1){//如果当前页数小于1，则赋�?�为1
			currentpage = 1;
		}else if(currentpage > sumpage){//如果当前页数大于总页数，则赋值为总页�?
			currentpage = sumpage;
		}
		//分页标签，开始循环数
		...
		//调整分页标签
		while (beginpage < 2) {//循环，一直到�?始循环数不小�?2
			if(beginpage != 1 && endpage < sumpage - 1){//�?始循环数 !=1，且 结束循环�? < 总页�?-1
				endpage++; //结束循环�?+1
			}
			beginpage++;//�?始循环数+1
		}
		while (endpage >= sumpage) {//循环，一直到结束循环�?>=总页�?
			if(endpage != sumpage && beginpage > 2){//结束循环�? != 总页�? �? �?始循环数 > 2
				beginpage--; //�?始循环数-1
			}
			endpage--; //结束循环�?-1
		}
